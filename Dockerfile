...
# vite.config.ts reads ../deno.jsonc for the app version.
...
# Use the frontend built inside the builder stage so the image always contains
# the current source (no reliance on a host-provided ./dist or a prebuilt image).
